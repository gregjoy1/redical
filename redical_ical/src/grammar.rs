...
/// Wraps an expected value in dquote
///
/// # Examples
///
/// ```rust
...
///
/// let Ok((remaining, parsed_quoted_value)) = result else {
///     panic!("Expected to be Ok, Actual: {:#?}", result);
/// };
///
/// assert_eq!(remaining.to_string(), String::from(" TESTING"));
/// assert_eq!(parsed_quoted_value, Quoted(Uri(String::from("mailto:jsmith@example.com"))));
/// ```
#[derive(Debug, Clone, Eq, PartialEq, Hash)]
...
impl<T> ICalendarEntity for Quoted<T>
where
    T: std::fmt::Debug + Clone + ICalendarEntity + Eq + PartialEq + std::hash::Hash,
{
    fn parse_ical(input: ParserInput) -> ParserResult<Self> {
