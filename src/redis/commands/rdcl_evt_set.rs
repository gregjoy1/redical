...
use crate::core::{Event, Calendar, CalendarIndexUpdater, EventDiff, InvertedEventIndex, rebase_overrides};
...
pub fn redical_event_set(ctx: &Context, args: Vec<RedisString>) -> RedisResult {
    ...
    let existing_event = calendar.events.get(&String::from(event_uuid.clone())).and_then(|event| Some(event.clone()));
