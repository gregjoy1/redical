...
pub fn parse_duration_string_components(input: &str) -> ParserResult<&str, (Option<&str>, Option<&str>, Option<(&str, Option<&str>, Option<&str>, Option<&str>)>)> {
    context(
        "parsed duration",
        preceded(
            tag("P"),
            ...
            Ok((remaining, parsed_duration_string_components)) => {
                if remaining.is_empty() {
                    ...
                    if let Some(weeks) = weeks {
                        let parsed_weeks =
                            str::parse::<i64>(weeks).map_err(|_error| {
                    ...
                    if let Some(days) = days {
                        let parsed_days =
                            str::parse::<i64>(days).map_err(|_error| {
                    ...
                    if let Some((_time_delim, hours, minutes, seconds)) = time_component {
                        ...
                        if let Some(hours) = hours {
                            let parsed_hours =
                                str::parse::<i64>(hours).map_err(|_error| {
                        ...
                        if let Some(minutes) = minutes {
                            let parsed_minutes =
                                str::parse::<i64>(minutes).map_err(|_error| {
                        ...
                        if let Some(seconds) = seconds {
                            let parsed_seconds =
                                str::parse::<i64>(seconds).map_err(|_error| {
...
mod test {
    ...
    fn test_parse_duration() {
        assert_eq!(
            ...
            Err(
                String::from("Unexpected values: '15--INVALID20S'")
            )
        );
        ...
        assert_eq!(
            ...
            Err(
                String::from("Unexpected values: ' SOMETHING ELSE'")
            )
        );
    ...
    fn test_get_duration_in_seconds() {
        assert_eq!(
            ParsedDuration::default().get_duration_in_seconds(),
            0
        );
    ...
    fn test_to_ical() {
        assert_eq!(
            ParsedDuration::default().to_ical(),
            None
        );
