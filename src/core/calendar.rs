...
use crate::core::inverted_index::{InvertedCalendarIndex, IndexedConclusion};

use crate::core::utils::{KeyValuePair, UpdatedHashMapMembers, UpdatedAttribute};

use crate::core::geo_index::{GeoSpatialCalendarIndex, GeoPoint};
...
impl <'a>CalendarIndexUpdater<'a> {
    ...
    pub fn update_indexed_geo(&mut self, updated_event_geo_diff: &UpdatedHashMapMembers<GeoPoint, IndexedConclusion>) -> Result<bool, String> {
        ...
        for (updated_long_lat_coord, updated_indexed_conclusion) in updated_event_geo_diff.updated.iter() {
            indexed_geo.insert(self.event_uuid.clone(), updated_long_lat_coord, updated_indexed_conclusion)?;
        }

        for (added_long_lat_coord, added_indexed_conclusion) in updated_event_geo_diff.added.iter() {
            indexed_geo.insert(self.event_uuid.clone(), added_long_lat_coord, added_indexed_conclusion)?;
        }
