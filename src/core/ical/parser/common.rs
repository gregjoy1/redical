...
impl<'a> ParsedValue<'a> {
    ...
    */
    ...
    pub fn expect_list(&'a self) -> Vec<&'a str> {
        if let ParsedValue::List(parsed_value) = self {
            parsed_value.clone()
        } else {
            panic!("Expected parsed list value, received {:#?}", self);
        }
    }

    pub fn expect_single(&'a self) -> &str {
        if let ParsedValue::Single(parsed_value) = self {
            parsed_value
        } else {
            panic!("Expected parsed single value, received {:#?}", self);
        }
    }

    pub fn expect_lat_long(&'a self) -> (f64, f64) {
        if let ParsedValue::LatLong(lat, long) = self {
            (*lat, *long)
        } else {
            panic!("Expected parsed lat/long value, received {:#?}", self);
        }
    }

    pub fn expect_geo_distance(&'a self) -> GeoDistance {
        if let ParsedValue::GeoDistance(parsed_value) = self {
            parsed_value.clone()
        } else {
            panic!("Expected parsed geo distance value, received {:#?}", self);
        }
    }

    pub fn expect_date_string(&'a self) -> ParsedDateString {
        if let ParsedValue::DateString(parsed_value) = self {
            parsed_value.clone()
        } else {
            panic!("Expected parsed date string value, received {:#?}", self);
        }
    }
    ...
    pub fn expect_timezone(&'a self) -> Tz {
        if let ParsedValue::TimeZone(parsed_value) = self {
            ...
            panic!("Expected parsed timezone value, received {:#?}", self);
        }
    }

    pub fn expect_duration(&'a self) -> ParsedDuration {
        if let ParsedValue::Duration(parsed_value) = self {
            parsed_value.clone()
        } else {
            panic!("Expected parsed duration value, received {:#?}", self);
        }
    }
}
...
pub fn params(input: &str) -> ParserResult<&str, HashMap<&str, &str>> {
    context(
        "params",
...
pub fn param(input: &str) -> ParserResult<&str, (&str, &str)> {
...
pub fn param_value(input: &str) -> ParserResult<&str, &str> {
    context("param value", alt((quoted_string, param_text)))(input)
}
...
pub fn is_language_char(chr: char) -> bool {
    is_alphanumeric(chr as u8) || chr == '-'
}

pub fn is_value_text_char(chr: char) -> bool {
    is_safe_char(chr) || chr == ':' || chr == '"'
}
