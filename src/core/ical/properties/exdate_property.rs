...
impl SerializableICalProperty for EXDateProperty {
    fn serialize_to_split_ical(&self) -> (String, Option<Vec<(String, String)>>, SerializedValue) {
        ...
        (String::from(EXDateProperty::NAME), params, value)
    }
}

impl EXDateProperty {
    const NAME: &'static str = "EXDATE";

    pub fn parse_ical(input: &str) -> ParserResult<&str, EXDateProperty> {
        preceded(
            tag("EXDATE"),
            cut(context(
                "EXDATE",
        ...
        .and_then(
            |(remaining, (parsed_params, _colon_delimeter, parsed_value)): (
                ...
                let parsed_property = EXDateProperty {
                    value_type,
                    timezone,
                    utc_timestamp,
                    x_params,
...
mod test {
    ...
    fn test_parse_ical_with_invalid_date_value_type() {
        let input = "EXDATE;VALUE=DATE:20201231T183000Z";
        let parsed_property = EXDateProperty::parse_ical(input);
    ...
    fn test_parse_ical_minimal() {
        assert_eq!(
            EXDateProperty::parse_ical("EXDATE:20201231T183000Z"),
            Ok((
                "",
                EXDateProperty {
                    value_type: None,
    ...
    fn test_parse_ical_full() {
        assert_eq!(
            EXDateProperty::parse_ical(
                r#"EXDATE;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000"#
            ),
            Ok((
                "",
                EXDateProperty {
    ...
    fn test_parse_ical_full_with_lookahead() {
        assert_eq!(
            EXDateProperty::parse_ical(
                r#"EXDATE;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000 SUMMARY:Summary text."#
            ),
            Ok((
    ...
    fn test_serialize_to_ical_with_timezone() {
        let parsed_property = EXDateProperty::parse_ical(
            r#"EXDATE;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000"#,
        ).unwrap().1;
        ...
        assert_eq!(
            EXDateProperty::parse_ical(serialized_ical.as_str())
                .unwrap()
                .1,
            parsed_property
        );
        ...
        assert_eq!(
            serialized_ical,
            String::from(
                r#"EXDATE;TZID=Europe/London;VALUE=DATE-TIME;X-TEST-KEY-ONE=VALUE_ONE,VALUE_TWO;X-TEST-KEY-TWO=KEY -🎄- TWO:20201231T183000"#,
            ),
        );
    }
    ...
    fn test_serialize_to_ical_with_no_timezone() {
        let parsed_property = EXDateProperty::parse_ical(
            r#"EXDATE;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000Z"#,
        ).unwrap().1;
        ...
        assert_eq!(
            serialized_ical,
            String::from(
                r#"EXDATE;VALUE=DATE-TIME;X-TEST-KEY-ONE=VALUE_ONE,VALUE_TWO;X-TEST-KEY-TWO=KEY -🎄- TWO:20201231T183000Z"#,
            ),
        );
    }
}
