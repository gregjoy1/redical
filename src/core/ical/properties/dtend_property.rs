...
impl SerializableICalProperty for DTEndProperty {
    fn serialize_to_split_ical(&self) -> (String, Option<Vec<(String, String)>>, SerializedValue) {
        ...
        (String::from(DTEndProperty::NAME), params, value)
    }
}

impl DTEndProperty {
    const NAME: &'static str = "DTEND";

    pub fn parse_ical(input: &str) -> ParserResult<&str, DTEndProperty> {
        preceded(
            tag("DTEND"),
            cut(context(
                "DTEND",
        ...
        .and_then(
            |(remaining, (parsed_params, _colon_delimeter, parsed_value)): (
                ...
                let parsed_property = DTEndProperty {
                    value_type,
                    timezone,
                    utc_timestamp,
                    x_params,
...
mod test {
    ...
    fn test_parse_ical_with_invalid_date_value_type() {
        let input = "DTEND;VALUE=DATE:20201231T183000Z";
        let parsed_property = DTEndProperty::parse_ical(input);
    ...
    fn test_parse_ical_minimal() {
        assert_eq!(
            DTEndProperty::parse_ical("DTEND:20201231T183000Z"),
            Ok((
                "",
                DTEndProperty {
                    value_type: None,
    ...
    fn test_parse_ical_full() {
        assert_eq!(
            DTEndProperty::parse_ical(
                r#"DTEND;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000"#
            ),
            Ok((
                "",
                DTEndProperty {
    ...
    fn test_parse_ical_full_with_lookahead() {
        assert_eq!(
            DTEndProperty::parse_ical(
                r#"DTEND;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000 SUMMARY:Summary text."#
            ),
            Ok((
    ...
    fn test_serialize_to_ical_with_timezone() {
        let parsed_property = DTEndProperty::parse_ical(
            r#"DTEND;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000"#,
        ).unwrap().1;
        ...
        assert_eq!(
            DTEndProperty::parse_ical(serialized_ical.as_str())
                .unwrap()
                .1,
            parsed_property
        );
        ...
        assert_eq!(
            serialized_ical,
            String::from(
                r#"DTEND;TZID=Europe/London;VALUE=DATE-TIME;X-TEST-KEY-ONE=VALUE_ONE,VALUE_TWO;X-TEST-KEY-TWO=KEY -🎄- TWO:20201231T183000"#,
            ),
        );
    }
    ...
    fn test_serialize_to_ical_with_no_timezone() {
        let parsed_property = DTEndProperty::parse_ical(
            r#"DTEND;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000Z"#,
        ).unwrap().1;
        ...
        assert_eq!(
            serialized_ical,
            String::from(
                r#"DTEND;VALUE=DATE-TIME;X-TEST-KEY-ONE=VALUE_ONE,VALUE_TWO;X-TEST-KEY-TWO=KEY -🎄- TWO:20201231T183000Z"#,
            ),
        );
    }
}
