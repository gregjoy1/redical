...
use nom::{
    ...
    error::{context, VerboseErrorKind, VerboseError},
...
pub fn serialize_timestamp_to_ical_datetime(utc_timestamp: &i64, timezone: &Tz) -> String {
    ...
    format!(
        "{}{}",
        serialized_datetime, timezone_postfix
    )
}
...
impl SerializableICalProperty for DTStartProperty {
    fn serialize_to_split_ical(&self) -> (String, Option<Vec<(String, String)>>, SerializedValue) {
        ...
        let value = SerializedValue::Single(serialize_timestamp_to_ical_datetime(&self.utc_timestamp, property_timezone));
    ...
    pub fn parse_ical(input: &str) -> ParserResult<&str, DTStartProperty> {
            ...
            cut(context(
                ...
                tuple((
                    build_property_params_parser!(
                        "DTSTART",
                        ("VALUE", common::ParsedValue::parse_single(alt((tag("DATE-TIME"), tag("DATE"))))),
        ...
        .and_then(
            |(remaining, (parsed_params, _colon_delimeter, parsed_value)): (
                &str,
                (Option<HashMap<&str, common::ParsedValue>>, &str, common::ParsedValue),
            )| {
                ...
                let utc_timestamp = match parsed_date_string.to_date(timezone, Self::NAME) {
                    ...
                    Err(_error) => {
                        return Err(nom::Err::Error(VerboseError {
                            errors: vec![(input, VerboseErrorKind::Context("parsed datetime value invalid"))],
                        }));
                    },
                };
                ...
                match value_type {
                    Some(ValueType::DateTime) if parsed_date_string.time.is_none() => {
                        return Err(nom::Err::Error(VerboseError {
                            errors: vec![(input, VerboseErrorKind::Context("expected parsed DATE-TIME value, received DATE"))],
                        }));
                    },

                    Some(ValueType::Date) if parsed_date_string.time.is_some() => {
                        return Err(nom::Err::Error(VerboseError {
                            errors: vec![(input, VerboseErrorKind::Context("expected parsed DATE value, received DATE-TIME"))],
                        }));
                    },
...
mod test {
    ...
    fn test_parse_ical_with_invalid_date_value_type() {
        ...
        let parsed_dtstart_property = DTStartProperty::parse_ical(input);

        assert!(parsed_dtstart_property.is_err());

        let nom::Err::Error(error) = parsed_dtstart_property.unwrap_err() else {
    ...
    fn test_parse_ical_with_invalid_date_string() {
        ...
        dbg!(&parsed_dtstart_property);
        assert!(parsed_dtstart_property.is_err());

        let nom::Err::Failure(error) = parsed_dtstart_property.unwrap_err() else {
    ...
    fn test_parse_ical_full() {
        assert_eq!(
            DTStartProperty::parse_ical(r#"DTSTART;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000"#),
            Ok((
                "",
                DTStartProperty {
    ...
    fn test_parse_ical_full_with_lookahead() {
        assert_eq!(
            DTStartProperty::parse_ical(r#"DTSTART;TZID=Europe/London;X-TEST-KEY-ONE=VALUE_ONE,"VALUE_TWO";VALUE=DATE-TIME;X-TEST-KEY-TWO="KEY -🎄- TWO":20201231T183000 SUMMARY:Summary text."#),
            Ok((
    ...
    fn test_serialize_to_ical_with_timezone() {
        ...
        let serialized_ical = parsed_dtstart_property.serialize_to_ical();
