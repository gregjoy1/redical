...
use crate::data_types::{CALENDAR_DATA_TYPE, Calendar, CalendarIndexUpdater, InvertedEventIndex};
...
pub fn redical_event_override_del(ctx: &Context, args: Vec<RedisString>) -> RedisResult {
    ...
    ctx.log_debug(format!("rdcl.evo_del: key: {calendar_uuid} event uuid: {event_uuid}").as_str());
